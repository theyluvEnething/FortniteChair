...
#include "driver.h"
#include "include/mappermain.hpp"
...
int mappermain() {
	...
	/*std::ofstream outfile("dateiname.bin", std::ios::out | std::ios::binary);
	if (outfile.is_open()) {
		outfile.write(reinterpret_cast<const char*>(raw_image.data()), raw_image.size());
		outfile.close();
		//Log(L"[+] Raw image data saved to driver_image.bin" << std::endl);
	}
	else {
		//Log(L"[-] Failed to save raw image data to file" << std::endl);
	}
	...
	NTSTATUS exitCode = 0;
	if (!kdmapper::MapDriver(iqvw64e_device_handle, drv.data(), 0, 0, free, true, mode, passAllocationPtr, callbackExample, &exitCode)) {
